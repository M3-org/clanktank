...
def insert_test_feedback(submission_id: str, feedback_data: list[tuple[str, str, str, float]] | None = None) -> None:
    """
...
def insert_test_research(
    submission_id: int, research_data: dict[str, Any] | None = None, version: str = DEFAULT_VERSION
) -> None:
    """
    ...
    if research_data is None:
        research_data = {
            ...
            "technical_analysis": {"complexity_score": 8.5, "innovation_score": 9.0, "feasibility_score": 7.5},
        }
...
def insert_test_scores(
    submission_id: int, scores_data: dict[str, float] | None = None, version: str = DEFAULT_VERSION
) -> None:
    """
...
def get_submission_status(submission_id: int, version: str = DEFAULT_VERSION) -> str | None:
    """
...
def update_submission_status(submission_id: int, new_status: str, version: str = DEFAULT_VERSION) -> bool:
    """
    ...
    try:
        cursor.execute(f"UPDATE {table_name} SET status = ? WHERE submission_id = ?", (new_status, submission_id))
        conn.commit()
...
def get_submission_with_scores(submission_id: int, version: str = DEFAULT_VERSION) -> dict[str, Any] | None:
    """
