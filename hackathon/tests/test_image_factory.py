...
from .test_constants import MAX_IMAGE_SIZE_MB, SMALL_IMAGE_SIZE, TEST_IMAGE_COLOR, TEST_IMAGE_SIZE, TEST_IMAGE_TEXT
...
def create_large_test_image(size: tuple[int, int] = (2000, 1500), color: str = "blue") -> io.BytesIO:
...
def create_test_image_with_transparency(
    size: tuple[int, int] = TEST_IMAGE_SIZE, background_color: str = "white"
) -> io.BytesIO:
...
def create_test_jpeg_image(size: tuple[int, int] = TEST_IMAGE_SIZE, quality: int = 85) -> io.BytesIO:
...
def get_image_info(image_bytes: io.BytesIO) -> dict:
    ...
    try:
        ...
        info = {"format": img.format, "mode": img.mode, "size": img.size, "width": img.width, "height": img.height}
...
def create_test_image_files_dict(image_bytes: io.BytesIO, filename: str = "test_image.png") -> dict:
    """
