...
def insert_test_submissions():
    ...
    try:
        ...
        for submission in TEST_SUBMISSIONS:
            ...
            cursor.execute(
                f"INSERT INTO {DEFAULT_TABLE} ({columns}) VALUES ({placeholders})", tuple(insert_data.values())
            )
...
def check_scoring_results():
    ...
    logger.info("-" * 80)
    current_sub = None
    for sub_id, name, judge, score, _created in scores:
    ...
    for i, (_sub_id, name, avg_score, judge_count) in enumerate(averages, 1):
...
def main():
    ...
    parser.add_argument("--reset", action="store_true", help="Just reset test data without running full pipeline")
    parser.add_argument("--check", action="store_true", help="Check current status of test submissions")
    parser.add_argument("--setup", action="store_true", help="Reset and insert test data")
