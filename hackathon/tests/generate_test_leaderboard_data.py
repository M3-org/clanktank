...
def generate_judge_scores(project_category, base_quality="good"):
    ...
    judges = {
        "aimarc": {
            ...
            "multipliers": {"innovation": 0.8, "technical_execution": 1.0, "market_potential": 1.3, "user_experience": 1.2}
        },
        "aishaw": {
            "strengths": ["technical_execution", "innovation"],
            "multipliers": {"innovation": 1.3, "technical_execution": 1.4, "market_potential": 0.9, "user_experience": 0.8}
        },
        "spartan": {
            "strengths": ["market_potential", "technical_execution"],
            "multipliers": {"innovation": 1.0, "technical_execution": 1.2, "market_potential": 1.4, "user_experience": 0.7}
        },
        "peepo": {
            "strengths": ["user_experience", "innovation"],
            "multipliers": {"innovation": 1.2, "technical_execution": 0.7, "market_potential": 1.0, "user_experience": 1.5}
        }
    }
...
def create_test_submissions_with_scores():
    ...
    for i, ((project, discord_user), quality) in enumerate(zip(submissions_data, quality_levels)):
...
def add_score_to_database(submission_id, judge_name, scores, weighted_total, comment):
    ...
    with engine.connect() as conn:
        ...
        synthesis_comment = f"After community feedback: {comment.split('.')[0].lower()}. " + \
                          ("Community response was very positive." if round2_adjustment > 0.2 else
                           "Mixed community reactions." if round2_adjustment > -0.2 else
                           "Community had some concerns.")
        ...
        for _ in range(random.randint(2, 8)):
            ...
            try:
                response = client.post(
                    f"/api/submissions/{submission['id']}/like-dislike",
                    json={"action": action},
                    headers=headers
                )
