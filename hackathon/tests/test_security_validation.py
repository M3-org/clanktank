...
class TestDatabaseConstraints:
    ...
    def test_valid_submission_with_constraints(self):
        ...
        cursor.execute(
            "SELECT project_name FROM hackathon_submissions_v2 WHERE submission_id = 'test-valid-constraint'"
        )
...
class TestAuditLogging:
    ...
    def test_security_event_logging(self):
        ...
        # Clean up
        cursor.execute(
            "DELETE FROM simple_audit WHERE action = ? AND user_id = ?", (f"security_{test_event}", test_user)
        )
        conn.commit()
        conn.close()
...
class TestUnauthorizedAccess:
    ...
    def test_unauthorized_edit_blocked(self):
        ...
        response = requests.put(
            f"{BASE_URL}/api/submissions/non-existent-submission",
            json={
