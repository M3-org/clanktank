...
from .test_constants import HTTP_BAD_REQUEST, HTTP_OK, VALID_CATEGORIES
...
def assert_valid_submission_structure(data: dict[str, Any], version: str = "v2") -> None:
    ...
    if version == "v2":
        required_fields.extend(["how_it_works", "problem_solved", "coolest_tech", "next_steps"])
...
def assert_valid_leaderboard_structure(data: dict[str, Any]) -> None:
    ...
    if data["submissions"]:
        ...
        required_fields = ["submission_id", "project_name", "team_name", "category", "total_score", "rank"]
...
def assert_valid_stats_structure(data: dict[str, Any]) -> None:
    ...
    expected_fields = ["total_submissions", "submissions_by_status", "submissions_by_category"]
...
def assert_valid_scores_structure(data: dict[str, Any]) -> None:
    ...
    expected_fields = ["aimarc_score", "aishaw_score", "peepo_score", "spartan_score", "community_score", "total_score"]
...
def assert_valid_feedback_structure(data: dict[str, Any]) -> None:
    ...
    required_fields = ["discord_user_id", "discord_user_nickname", "reaction_type", "score_adjustment"]
...
def assert_database_state(submission_id: int, expected_status: str | None = None, should_exist: bool = True) -> None:
    """Assert database state for a submission"""
    from .test_db_helpers import get_submission_status
...
def assert_submission_progression(submission_id: int, expected_stages: list[str]) -> None:
...
def assert_no_duplicate_submissions(project_name: str, team_name: str) -> None:
    ...
    matches = [s for s in submissions if s["project_name"] == project_name and s["team_name"] == team_name]
