...
class EpisodeGeneratorV2:
    ...
    def fetch_project_data(self, submission_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(
                f"Could not fetch submission {submission_id} from API. Ensure the backend is running at {self.config.api_base_url}"
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default=config.episode_output_dir, help="Output directory for generated episodes"
    )
    parser.add_argument(
