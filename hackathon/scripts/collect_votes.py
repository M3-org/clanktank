...
class TokenHolderRegistry:
    ...
    def __init__(self, holders_csv_path: str | None = None):
    ...
    def _load_holders(self):
        ...
        try:
            with open(self.holders_csv_path) as f:
...
class HeliusClient:
    ...
    async def get_enhanced_transactions(self, signatures: list[str]) -> list[dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session, session.post(url, json=payload, timeout=25) as response:
    ...
    async def stream_wallet_transfers(
        self, since_unix: int | None = None, page_size: int = 100
    ) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            async with aiohttp.ClientSession() as session, session.get(url, timeout=25) as response:
                if response.status != 200:
    ...
    async def get_enhanced_transactions_by_address(
        self, address: str, limit: int = 100, tx_type: str | None = None
    ) -> list[dict[str, Any]]:
    ...
    def extract_memo(self, tx: dict[str, Any]) -> str | None:
        ...
        if (memos := tx.get("memos")) and isinstance(memos, list) and memos:
    ...
    def extract_memo_from_transaction(self, tx_data: dict[str, Any]) -> str | None:
        ...
        for instruction in instructions:
            ...
            if instruction.get("programId") == MEMO_PROGRAM_ID:
                self.logger.info("🎯 Found memo program instruction!")
    ...
    def extract_token_transfer_amount(self, tx_data: dict[str, Any]) -> int:
        ...
        for transfer in token_transfers:
            if transfer.get("mint") == ai16z_mint and transfer.get("toUserAccount") == self.prize_wallet:
    ...
    def is_voting_transaction(self, tx_data: dict[str, Any]) -> bool:
        ...
        return any(transfer.get("toUserAccount") == self.prize_wallet for transfer in native_transfers)

    def extract_sender(self, tx: dict[str, Any]) -> str | None:
    ...
    def is_submission_id_memo(self, memo: str) -> bool:
        ...
        return bool(re.match(r"^[a-zA-Z0-9_-]+$", memo.strip()))
    ...
    async def find_ata_address(self, wallet: str, mint: str) -> str:
        ...
        async with aiohttp.ClientSession() as session, session.post(rpc_url, json=payload) as response:
    ...
    async def detect_voting_transactions(
        self, since_timestamp: int | None = None, holders_registry=None
    ) -> list[CommunityVote]:
        ...
        try:
            ...
            if ata_address:
                ...
                self.logger.info("Could not find ATA, using wallet address")
            ...
            async with aiohttp.ClientSession() as session, session.get(full_url) as response:
                if response.status != 200:
                    error_text = await response.text()
    ...
    async def detect_voting_transactions_fallback(
        self, since_timestamp: int | None = None, holders_registry=None
    ) -> list[CommunityVote]:
        ...
        with self.get_db_connection() as conn:
            ...
            for row in cursor.fetchall():
                ...
                submission_votes[submission_id].append(
                    {"sender": sender_address, "amount": amount, "timestamp": timestamp}
                )
            ...
            for submission_id, votes in submission_votes.items():
                ...
                for vote in votes:
                    ...
                    if self.holders_registry.is_token_holder(sender):
                        ...
                        self.logger.debug(
                            f"Vote from {sender[:8]}...: balance={self.holders_registry.get_balance(sender):.2f}, weight={quadratic_weight:.2f}"
                        )
                    else:
                ...
                community_score = min(math.log10(quadratic_score + 1) * 2, 10) if quadratic_score > 0 else 0
  ...
  python collect_votes.py --process-tx TX_SIGNATURE # Process and save specific transaction
    ...
    try:
        ...
        if args.test and helius_client:
            ...
            signatures = await helius_client.get_signatures_for_address(helius_client.prize_wallet, limit=5)
            ...
            if signatures:
                ...
                for i, sig in enumerate(signatures[:3]):
                    ...
                    time_str = (
                        datetime.fromtimestamp(block_time).strftime("%Y-%m-%d %H:%M:%S") if block_time else "unknown"
                    )
        ...
        if args.debug_tx and helius_client:
            ...
            try:
                ...
                if sender and processor.holders_registry.is_token_holder(sender):
                    ...
                    print(
                        f"   Token holder: ✅ Balance: {voting_info['balance']:.2f}, Weight: {voting_info['quadratic_weight']:.2f}"
                    )
                else:
                    print("   Token holder: ❌")
        ...
        if args.process_tx and helius_client:
            ...
            try:
                ...
                if saved_count > 0:
                    ...
                    print(
                        f"   Voter: {sender[:8]}... (Balance: {voting_info['balance']:.2f}, Weight: {voting_info['quadratic_weight']:.2f})"
                    )
                    print(f"   Amount: {amount / 1_000_000:.6f} tokens")
                    print(f"   Transaction: {args.process_tx[:20]}...")
                else:
                    print("⚠️  Vote already exists in database")
        ...
        if args.collect and helius_client:
            ...
            votes = await helius_client.detect_voting_transactions(
                since_timestamp=args.since, holders_registry=processor.holders_registry
            )
            ...
            if votes:
                ...
                if saved_count > 0:
                    ...
                    for vote in votes:
                        ...
                        print(
                            f"  📝 {vote.submission_id}: {vote.amount / 1_000_000:.6f} tokens from {vote.sender_address[:8]}... (weight: {weight:.2f})"
                        )
            else:
