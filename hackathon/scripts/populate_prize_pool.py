    ...
    try:
        ...
        for asset in data['result']['items']:
            if asset.get('token_info') and float(asset['token_info'].get('balance', 0)) > 0:
                ...
                symbol = asset['token_info'].get('symbol') or asset.get('content', {}).get('metadata', {}).get('symbol') or asset['id'][:8]
...
def main():
    ...
    print(f"📊 You can now test the /api/prize-pool endpoint with real data")
