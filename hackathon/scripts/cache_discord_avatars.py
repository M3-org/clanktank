...
PRODUCTION_BASE_URL = os.getenv("PRODUCTION_BASE_URL", "https://clanktank.tv")
...
def generate_fallback_avatar(username, cache_path):
    ...
    try:
        ...
        result = subprocess.run(
            ["convert", str(svg_path), "-resize", "128x128", str(cache_path)], capture_output=True, text=True
        )
...
def update_user_avatar_url(conn, discord_id, avatar_filename, update_generated_only=False):
    """Update user's avatar URL in database with production URL."""
    try:
        production_url = f"{PRODUCTION_BASE_URL}{DISCORD_PATH}{avatar_filename}"

        if update_generated_only:
            # Only update if user doesn't already have a custom Discord CDN avatar
            cursor = conn.cursor()
            cursor.execute(
                """
                UPDATE users
                SET avatar = ?
                WHERE discord_id = ?
                AND (avatar IS NULL OR avatar = '' OR NOT avatar LIKE 'https://cdn.discordapp.com/avatars/%')
            """,
                (production_url, discord_id),
            )
        else:
            # Update avatar URL regardless (for custom cached avatars)
            cursor = conn.cursor()
            cursor.execute(
                """
                UPDATE users
        ...
        if cursor.rowcount > 0:
            logger.debug(f"Updated avatar URL for {discord_id}: {production_url}")
            return True
        else:
            logger.debug(f"No avatar URL update needed for {discord_id}")
            return False

    except Exception as e:
        logger.error(f"Error updating avatar URL for {discord_id}: {e}")
        return False
...
def main():
    ...
    try:
        ...
        for discord_id, avatar_url, username in users:
            try:
                ...
                if has_custom_avatar:
                    ...
                    if update_user_avatar_url(conn, discord_id, cache_filename):
                        db_updated_count += 1
                    ...
                    if generate_fallback_avatar(username, cache_path):
                        ...
                        # Update database with production URL (only for users without custom avatars)
                        if update_user_avatar_url(conn, discord_id, cache_filename, update_generated_only=True):
                            db_updated_count += 1
                    else:
                        error_count += 1
        ...
        logger.info(
            f"✅ Complete! Cached {cached_count} custom avatars, generated {generated_count} fallback avatars, updated {db_updated_count} database records, {error_count} errors"
        )
